                      ...
                      <div className="md:col-span-1 flex items-center justify-end gap-1">
                        <button 
                          className={`cursor-pointer p-2.5 border transition-all ${
                            isDark 
                              ? 'hover:bg-green-500/10 text-green-400 border-white/10 hover:border-green-500/20' 
                              : 'hover:bg-green-50 text-green-600 border-neutral-200 hover:border-green-300'
                          }`} 
                          title="Add Animal"
                          onClick={() => {
                            router.push(`/livestockmanagement/animal/animals?addAnimal=true&shedId=${shed.id}&shedName=${encodeURIComponent(shed.shedName || shed.name)}`);
                          }}
                        >
                          <Plus className="w-4 h-4" />
                        </button>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="relative">
              ...
              <input
                ...
                onFocus={() => {
                  if (formData.farmLocation && formData.farmLocation.length >= 3) {
                    setShowLocationSuggestions(true);
                  }
                }}
              ...
              />
              {isFetchingLocation && (
                <div className={`absolute right-4 top-[52px] ${isDark ? 'text-green-500' : 'text-green-600'}`}>
                  <div className="w-5 h-5 border-2 border-t-transparent rounded-full animate-spin border-current"></div>
                </div>
              )}
              {showLocationSuggestions && locationSuggestions.length > 0 && (
                <div className={`absolute z-10 w-full mt-1 border shadow-lg max-h-60 overflow-auto ${
                  isDark ? 'bg-neutral-800 border-white/10' : 'bg-white border-neutral-200'
                }`}>
                  {locationSuggestions.map((loc) => (
                    <div
                      key={loc.place_id}
                      onClick={() => handleLocationSelect(loc)}
                      className={`cursor-pointer px-4 py-3 text-sm font-medium transition-colors border-b last:border-0 ${
                        isDark 
                          ? 'border-white/5 hover:bg-white/10 text-neutral-300' 
