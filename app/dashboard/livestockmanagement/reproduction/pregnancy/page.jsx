...
// Fetch Options for Dropdowns
  ...
  const fetchRecords = async () => {
    try {
      setLoading(true);
      const data = await reproductionService.getPregnancies();
      
      if (data && data.success) {
        setPregnancyRecords(data.data);
        localStorage.setItem('pregnancyRecords', JSON.stringify(data.data));
      } else {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (editingRecord) {
        ...
        const data = await reproductionService.updatePregnancy(editingRecord.id || editingRecord._id, updateData);
        if (data && data.success) {
          await fetchRecords();
        }
      } else {
        ...
        const data = await reproductionService.createPregnancy(newRecord);
        if (data && data.success) {
          await fetchRecords();
        }
      }
  ...
  const confirmDelete = async () => {
    ...
    try {
      await reproductionService.deletePregnancy(recordToDelete);
      await fetchRecords();
    } catch (error) {
