...
export default function BreedingRecords() {
  ...
  const fetchRecords = async () => {
    try {
      setLoading(true);
      const data = await reproductionService.getBreedings();

      if (data && data.success && Array.isArray(data.data)) {
        setBreedingRecords(data.data);
        localStorage.setItem('breedingRecords', JSON.stringify(data.data));
      } else if (data && Array.isArray(data)) {
        setBreedingRecords(data);
        localStorage.setItem('breedingRecords', JSON.stringify(data));
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (editingRecord) {
        ...
        const data = await reproductionService.updateBreeding(editingRecord.id || editingRecord._id, updateData);
        if (data && data.success) {
          await fetchRecords();
        }
      } else {
        const data = await reproductionService.createBreeding(formData);
        if (data && data.success) {
          await fetchRecords();
        }
      }
  ...
  const confirmDelete = async () => {
    ...
    try {
      await reproductionService.deleteBreeding(recordToDelete);
      await fetchRecords();
    } catch (error) {
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            ...
            <div className={`flex p-1.5 border backdrop-blur-md ${isDark ? 'bg-neutral-900/50 border-white/10' : 'bg-white border-neutral-300 shadow-sm'
              ...
              <Link href="/dashboard/livestockmanagement/reproduction/dashboard">
                <button
                  className={`cursor-pointer px-6 py-2.5 text-[11px] font-bold uppercase tracking-wider transition-all ${isActive('/dashboard/livestockmanagement/reproduction/dashboard')
                      ? isDark
              ...
              <Link href="/dashboard/livestockmanagement/reproduction/dryoff">
                <button
                  className={`cursor-pointer px-6 py-2.5 text-[11px] font-bold uppercase tracking-wider transition-all ${isActive('/dashboard/livestockmanagement/reproduction/dryoff')
                      ? isDark
                        ? 'bg-green-500/10 text-green-400 border border-green-500/20'
                        : 'bg-green-500/10 text-green-700 border border-green-500/30'
                      : isDark
