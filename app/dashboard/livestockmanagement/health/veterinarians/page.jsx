...
export default function VeterinariansManagement() {
  ...
  const fetchVeterinarians = async () => {
    try {
      ...
      if (data && data.success && Array.isArray(data.data)) {
        ...
        localStorage.setItem('livestockVeterinarians', JSON.stringify(data.data));
      } else if (data && Array.isArray(data)) {
        setVeterinarians(data);
        localStorage.setItem('livestockVeterinarians', JSON.stringify(data));
      } else {
  ...
  const validatePhone = (phone) => {
    ...
    // Regex for: optional +, followed by 10-15 digits/spaces/dashes
    const phoneRegex = /^\+?[\d\s-]{10,15}$/;
    if (!phoneRegex.test(phone)) {
      setPhoneError('Invalid phone format (e.g., +1 234 567 8900)');
      return false;
    }
    setPhoneError('');
    return true;
  ...
  const fetchAddressSuggestions = async (query) => {
    ...
    try {
      ...
      if (data && data.features) {
        ...
        // Remove duplicates
        setAddressSuggestions([...new Set(suggestions)]);
      }
    } catch (error) {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      if (formData.address && !addressSuggestions.includes(formData.address)) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.name || !isPhoneValid || !formData.address || !formData.specialization) {
      return;
    }
    ...
    try {
      if (editingVet) {
        const data = await healthService.updateVeterinarian(editingVet.id || editingVet._id, formData);
        if (data && data.success) {
          await fetchVeterinarians();
        }
      } else {
        const data = await healthService.createVeterinarian(formData);
        if (data && data.success) {
          await fetchVeterinarians();
        }
      }
  ...
  const handleDelete = async (id) => {
    try {
      await healthService.deleteVeterinarian(id);
      await fetchVeterinarians();
    } catch (error) {
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              <input
                ...
                className={`w-full px-4 py-3.5 border outline-none transition-all font-medium ${isDark
                    ? `bg-neutral-900 ${phoneError ? 'border-red-500/50' : 'border-white/10'} focus:border-green-500 placeholder:text-neutral-600`
                    : `bg-neutral-50 ${phoneError ? 'border-red-300' : 'border-neutral-300'} focus:border-green-500 placeholder:text-neutral-400`
                  }`}
                disabled={submitting}
              />
              {phoneError && (
                <p className="mt-2 text-[10px] font-bold text-red-500 uppercase tracking-wider font-mono">
                  [!] {phoneError}
                </p>
              )}
            </div>
            ...
            <div className="relative">
              ...
              {addressSuggestions.length > 0 && (
                <div className={`absolute left-0 right-0 mt-1 z-50 border shadow-2xl backdrop-blur-md overflow-hidden ${isDark ? 'bg-neutral-900/95 border-white/10' : 'bg-white/95 border-neutral-200'
                  }`}>
                  {addressSuggestions.map((suggestion, idx) => (
                    <button
                      ...
                      className={`cursor-pointer w-full text-left px-4 py-3 text-[11px] font-bold transition-colors border-b last:border-0 ${isDark
                          ? 'border-white/5 hover:bg-white/5 text-neutral-300'
