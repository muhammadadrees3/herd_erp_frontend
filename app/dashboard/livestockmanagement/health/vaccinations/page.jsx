...
export default function VaccinationsManagement() {
  ...
  const [statuses, setStatuses] = useState(['Administered', 'Overdue', 'Partially Completed']);
  ...
  const fetchVaccinations = async () => {
    try {
      ...
      if (data && data.success && Array.isArray(data.data)) {
        ...
        localStorage.setItem('livestockVaccinations', JSON.stringify(data.data));
      } else if (data && Array.isArray(data)) {
        setVaccinations(data);
        localStorage.setItem('livestockVaccinations', JSON.stringify(data));
      } else {
  ...
  // Fetch Statuses from API (Mock or API)
  const fetchStatuses = async () => {
    // Disabled as requested, using hardcoded statuses: 'Administered', 'Overdue', 'Partially Completed'
  };
  ...
  const fetchOptions = async () => {
    try {
      const [animalsData, vaccinesData, vetsData] = await Promise.all([
      ...
      if (vaccinesData && vaccinesData.success && Array.isArray(vaccinesData.data)) {
  ...
  const handleOpenForm = (vaccination = null) => {
    ...
    if (vaccination) {
      ...
      setFormData({
        ...
        status: vaccination.status || 'Administered',
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.status.toLowerCase() === 'partially completed' && !formData.nextDueDate) {
      setErrorMsg('Next Due Date is required when status is "Partially Completed".');
      return;
    }
    ...
    // Ensure nextDueDate has a value for the backend even if disabled in UI
    const apiData = {
      ...formData,
      nextDueDate: formData.nextDueDate || formData.vaccinationLogDate
    };
    
    try {
      if (editingVaccination) {
        const data = await healthService.updateVaccination(editingVaccination.id || editingVaccination._id, apiData);
        if (data && data.success) {
          await fetchVaccinations();
        }
      } else {
        const data = await healthService.createVaccination(formData);
        if (data && data.success) {
          await fetchVaccinations();
        }
      }
                  ...
                  <div key={vaccination.id || vaccination._id || Math.random()} className={`relative p-6 border transition-all duration-300 hover:-translate-y-1 ${
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-center">
                      ...
                      <div className="md:col-span-2">
                        ...
                        <p className={`text-sm font-medium ${isDark ? 'text-neutral-300' : 'text-neutral-700'}`}>
                          {vaccination.vaccinationLogDate || vaccination.dateGiven}
                        </p>
                      </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              >
                ...
                {vaccines.map((v, idx) => {
                  const val = typeof v === 'object' ? (v.name || v.vaccine || v.id || '') : v;
                  return (
                    <option key={idx} value={val}>
                      {val}
                    </option>
            ...
            <div>
              ...
              <input
                ...
                required
                readOnly
                value={formData.vaccinationLogDate}
                className={`w-full px-4 py-3.5 border outline-none transition-all font-medium opacity-70 cursor-not-allowed ${
                  isDark 
            ...
            <div>
              ...
              <input
                ...
                className={`w-full px-4 py-3.5 border outline-none transition-all font-medium ${
                  formData.status.toLowerCase() !== 'partially completed' ? 'opacity-70 cursor-not-allowed' : ''
                } ${
                  isDark 
                ...
                disabled={submitting || formData.status.toLowerCase() !== 'partially completed'}
              />
            </div>
            {/* Administered By */}
            <div>
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                ...
                <div>
                  ...
                  <p className="text-sm font-medium">{viewingVaccination.vaccinationLogDate || viewingVaccination.dateGiven}</p>
                </div>
