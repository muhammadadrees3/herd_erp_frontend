...
export default function OTCSRecords() {
  ...
  const fetchRecords = async () => {
    try {
      ...
      const data = await milkService.getOTCRecords(params);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingRecord) {
        const data = await milkService.updateOTCRecord(editingRecord.id || editingRecord._id, apiData);
        if (data && data.success) {
          await fetchRecords();
          setSearchTerm('');
        }
      } else {
        const data = await milkService.createOTCRecord(apiData);
        if (data && data.success) {
          await fetchRecords();
          setSearchTerm('');
        }
  ...
  const handleDelete = async (id) => {
    try {
      setSubmitting(true);
      await milkService.deleteOTCRecord(id);
      await fetchRecords();
