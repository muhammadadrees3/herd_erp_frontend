...
export default function ProductionAnalytics() {
  ...
  const fetchAnalytics = async (withDateFilter = false) => {
    try {
      ...
      const data = await milkService.getProductionAnalytics(params);
      
      if (data && data.success) {
        setAnalyticsData(data.data);
        localStorage.setItem('milkProductionAnalytics', JSON.stringify(data.data));
      } else {
  ...
  const handleDelete = async (date) => {
    ...
    try {
      const data = await milkService.getDailyProductionRecords({ 
        startDate: date,
        endDate: date 
      });
      
      if (data && data.success && data.data) {
        const recordsToDelete = data.data;
        
        const deletePromises = recordsToDelete.map(record => 
          milkService.deleteDailyProductionRecord(record.id || record._id)
        );
