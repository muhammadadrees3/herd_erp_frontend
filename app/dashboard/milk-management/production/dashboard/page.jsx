...
export default function MilkProductionDashboard() {
  ...
  const fetchDashboardStats = async () => {
    try {
      ...
      const data = await milkService.getProductionDashboardStats();
      
      if (data && data.success) {
        const stats = {
          todayTotal: data.data?.todayTotal || 0,
          animalCount: data.data?.animalCount || 0,
          averagePerAnimal: data.data?.averagePerAnimal || 0,
          yesterdayTotal: data.data?.yesterdayTotal || 0,
