...
export default function DailyProductionRecords() {
  ...
  const fetchRecords = async (useDateFilter = false) => {
    try {
      ...
      const data = await milkService.getDailyProductionRecords(params);
      
      if (data && data.success && Array.isArray(data.data)) {
        setRecords(data.data);
        localStorage.setItem('milkProductionRecords', JSON.stringify(data.data));
      } else if (data && Array.isArray(data)) {
        setRecords(data);
        localStorage.setItem('milkProductionRecords', JSON.stringify(data));
      } else {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingRecord) {
        ...
        const data = await milkService.updateDailyProductionRecord(editingRecord.id || editingRecord._id, apiData);
        ...
        // Add new record
        const data = await milkService.createDailyProductionRecord(apiData);
  ...
  const handleDelete = async (id) => {
    try {
      await milkService.deleteDailyProductionRecord(id);
      await fetchRecords(false);
    } catch (error) {
