...
export default function MilkUsage() {
  ...
  const fetchRecords = async () => {
    try {
      ...
      const data = await milkService.getUsageRecords(params);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingRecord) {
        const data = await milkService.updateUsageRecord(editingRecord.id || editingRecord._id, payload);
        if (data && data.success) {
          await fetchRecords();
          setFilterTime('All');
          setSearchTerm('');
        ...
        const data = await milkService.createUsageRecord(payload);
        if (data && data.success) {
          await fetchRecords();
          setFilterTime('All');
          setSearchTerm('');
  ...
  const handleDelete = async (id) => {
    try {
      setSubmitting(true);
      await milkService.deleteUsageRecord(id);
      await fetchRecords();
