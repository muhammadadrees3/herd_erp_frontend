...
export default function SalesRecords() {
  ...
  const fetchRecords = async () => {
    try {
      ...
      const data = await milkService.getSalesRecords(params);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingRecord) {
        const data = await milkService.updateSalesRecord(editingRecord.id || editingRecord._id, apiData);
        if (data && data.success) {
          await fetchRecords();
          setSearchTerm('');
          setFilterType('All');
        ...
        const data = await milkService.createSalesRecord(apiData);
        if (data && data.success) {
          await fetchRecords();
          setSearchTerm('');
          setFilterType('All');
  ...
  const handleDelete = async (id) => {
    try {
      setSubmitting(true);
      await milkService.deleteSalesRecord(id);
      await fetchRecords();
